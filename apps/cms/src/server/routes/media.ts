...
import { deleteMedia, listMedia, readMedia, saveMedia, SLUG_PATTERN } from '@codeblin/content';
...
media.get('/:slug/media', (context) => {
  ...
  return context.json({ files: listMedia(slug) });
});
...
media.get('/:slug/media/:file', (context) => {
  ...
  try {
    const bytes = readMedia(slug, file);
...
media.post('/:slug/media', async (context) => {
  ...
  if (file.size > MAX_BYTES) return context.json({ error: 'File exceeds 8 MB.' }, 413);

  const bytes = Buffer.from(await file.arrayBuffer());
  try {
    const src = saveMedia(slug, file.name, bytes);
    return context.json({ src, files: listMedia(slug) }, 201);
  } catch (error) {
...
media.delete('/:slug/media/:file', (context) => {
  ...
  const file = context.req.param('file');
  try {
    deleteMedia(slug, file);
    return context.json({ ok: true, files: listMedia(slug) });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Not found';
    return context.json({ error: message }, 404);
  }
